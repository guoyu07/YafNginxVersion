...
class Service_ShortUrl {
    ...
    public function shorten($url) {
        ...
        if (empty($id)) {
            ...
            if (intval($id) <= 0) {
                Base_Log::warning('allocate id failed', Admin_Conf_ErrorCode::ERROR_REDIS_SET_FAILED, array('id' => $id, 'url' => $url));
                return false;
            }
            //写入数据库Redis 
            $ret = self::$daoShortUrl->shorten($url, $id); 
            if($ret === false) {
                Base_Log::warning('craete short url failed', Admin_Conf_ErrorCode::ERROR_REDIS_SET_FAILED, array('id' => $id, 'url' => $url));
                return false;
            }
        }
        $result = array(
    ...
    public function custom($url, $customId) {
        ...
        //短链不存在，且需要定制关键词短链
            ...
            if ($ret) {
                Base_Log::warning('id have exists, should be custom id', Admin_Conf_ErrorCode::ERROR_REDIS_SET_FAILED, array('id' => $customId, 'url' => $url));
                return 10001;
            }
            ...
            if (!$shortRet) {
                Base_Log::warning('create short url failed', Admin_Conf_ErrorCode::ERROR_REDIS_SET_FAILED, array('id' => $customId, 'url' => $url));
                return false;
            }
